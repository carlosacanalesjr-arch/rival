...
export default function AdminDayList({ programId, levelKey, week }) {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-muted-2">Pick a day to edit.</p>
      <div className="mt-4 grid grid-cols-2 gap-2">
        {Array.from({ length: dayCount }, (_, i) => i + 1).map((day) => (
          <Link
            key={day}
            href={`/admin/${programId}/${levelKey}/${week}/${day}`}
            className="rounded-xl border border-border-subtle bg-surface p-3 text-center text-sm font-bold text-foreground transition hover:border-rival-red/40"
          >
            Day {day}
          </Link>
        ))}
