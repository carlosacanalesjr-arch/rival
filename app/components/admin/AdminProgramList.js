"use client";

import Link from "next/link";
import { usePrograms } from "@/app/lib/ProgramsContext";

export default function AdminProgramList() {
  const { programs } = usePrograms();

  const byCategory = {};
  for (const p of programs) {
    if (!byCategory[p.category]) byCategory[p.category] = [];
    byCategory[p.category].push(p);
  }

  return (
    <div>
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-extrabold text-foreground">Programs</h1>
          <p className="mt-1 text-sm text-muted-2">Pick a program to edit its exercises.</p>
        </div>
        <Link
          href="/admin/import"
          className="shrink-0 rounded-full bg-rival-red px-3.5 py-2 text-xs font-bold text-white hover:bg-red-600"
        >
          Import Spreadsheet
        </Link>
      </div>
      <div className="mt-4 space-y-6">
        {Object.entries(byCategory).map(([category, list]) => (
          <div key={category}>
            <p className="text-xs font-bold uppercase tracking-wide text-muted-2">{category}</p>
            <div className="mt-2 space-y-2">
              {list.map((p) => (
                <Link
                  key={p.id}
                  href={`/admin/${p.id}`}
                  className="block rounded-xl border border-border-subtle bg-surface p-3 text-sm font-bold text-foreground transition hover:border-rival-red/40"
                >
                  {p.title}
                </Link>
              ))}
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
